...
public interface BoardRepository extends JpaRepository<BoardEntity, Long> {
	...
	// :boardNo=@Param("boardNo") 과 매칭이 된다
	// 업데이트나 딜리트 쿼리를 수행할 때는 추가적으로 @Modifying 필수로 붙여야 한다
	@Modifying
	@Query(value="update BoardEntity b set b.boardHits=b.boardHits+1 where b.boardNo = :boardNo")
	void boardUpdateHits(@Param("boardNo") Long boardNo);
